...
year = 2022
for month in tqdm(range(4, 10)):
    # 拿orderbook+trade的数据
    # data_type = ''
    filters = [('symbol', '=', symbol), ('year', '=', year), ('month','=',month)]
    depth_base = pq.ParquetDataset('datafile/tick/order_book_100ms/binance_swap_u', filters=filters, filesystem=minio)
    depth = depth_base.read_pandas().to_pandas()
    depth = depth.iloc[:, :-5]
    depth = depth.sort_values(by='closetime', ascending=True)

    trade_base = pq.ParquetDataset('datafile/tick/trade/binance_swap_u', filters=filters, filesystem=minio, schema=schema)
...
def depth_price_skew(depth, trade):
    prices = ["bid_price10", "bid_price9", "bid_price8", "bid_price7", "bid_price6", "bid_price5", "bid_price4", "bid_price3", "bid_price2", "bid_price1", "ask_price1", "ask_price2",
              "ask_price3", "ask_price4", "ask_price5", "ask_price6", "ask_price7", "ask_price8", "ask_price9", "ask_price10"]
...
def avg_turnover(depth, trade, rolling=500):
    return depth[
        ['ask_size1', 'ask_size2', 'ask_size3', 'ask_size4', "ask_size5", "ask_size6", "ask_size7", "ask_size8", "ask_size9", "ask_size10",
         'bid_size1', 'bid_size2', 'bid_size3', 'bid_size4', "bid_size5", "bid_size7", "bid_size7", "bid_size8", "bid_size9", "bid_size10"]].sum(axis=1)
...
#%%
trade_factor['datetime'] = pd.to_datetime(trade_factor['closetime']+28800000, unit='ms')
trade = trade_factor.set_index('datetime').groupby(pd.Grouper(freq='60000ms')).apply('last')
add_factor['datetime'] = pd.to_datetime(add_factor['closetime']+28800000, unit='ms')
add = add_factor.set_index('datetime').groupby(pd.Grouper(freq='60000ms')).apply('last')

list_of_datasets = [add, trade]
data = reduce(
...
data.sort_values(by='closetime', ascending=True, inplace=True)
data['datetime'] = pd.to_datetime(data['closetime']+28800000, unit='ms')
data = data.set_index('datetime')
del add, trade
#%%
# depth_factor = reduce_mem_usage(depth_factor)[0]
# add_factor = reduce_mem_usage(add_factor)[0]
trade_factor = reduce_mem_usage(trade_factor)[0]
#%%
# depth_factor.to_csv('/run/media/ps/data/songhe/crypto/feat/btcusdt_2022_920_926_depth_factor.csv')
trade_factor.to_csv('/run/media/ps/data/songhe/crypto/feat/btcusdt_2022_920_926_trade_factor.csv')
# add_factor.to_csv('/run/media/ps/data/songhe/crypto/feat/btcusdt_2022_920_926_add_factor.csv')
#%%
trade_high_low['datetime'] = pd.to_datetime(trade_high_low['closetime']+28800000, unit='ms')
price_max_min = trade_high_low.set_index('datetime').groupby(pd.Grouper(freq='10000ms')).agg({'price':['min','max'], 'closetime':'last'})
price_max_min.columns = ['_'.join(col) for col in price_max_min.columns]
price_max_min = price_max_min[(price_max_min['closetime_last'] >= all_data['closetime'].iloc[1]) & (price_max_min['closetime_last'] <= all_data['closetime'].iloc[-1])]
price_max_min = price_max_min.rename({'closetime_last':'closetime'},axis='columns')
#%%
all_data['datetime'] = pd.to_datetime(all_data['closetime']+28800000, unit='ms')
df = all_data.set_index('datetime').groupby(pd.Grouper(freq='1000ms')).apply('last')
#%%
df = pd.merge(df, price_max_min, on='closetime', how='outer')
df = df.sort_values(by='closetime', ascending=True)
 #%%
# df['wap1'] = calc_wap1(df)
# df['buy_success'] = (df['wap1'].shift(1))-df['price_min']
# df['sell_success'] = df['price_max']-(df['wap1'].shift(1))
df['trade_success'] = np.where((df['price'].shift(1)<df['price_max'])&(df['price'].shift(1)>df['price_min']), 1, 0)
print(df['trade_success'].value_counts())
# df['buy_success'] = np.where(df['buy_success']>0, 1, df['buy_success'])
# df['sell_success'] = np.where(df['sell_success']>0, 1, df['sell_success'])
# df['target'] = df['buy_success']*df['sell_success']
#%%
# test['target'] = np.where(test['bid_price1'].shift(1)>test['bid_price1'],1,0)
# test['target'] = test['bid_price1'].shift(1)-test['bid_price1']
df['target'] = np.log(df['price']/df['price'].shift(1))
#%%
def classify(y):
    ...
    if y > 0:
        ...
        return -1
df['target'] = df['target'].apply(lambda x:classify(x))
print(df['target'].value_counts())
print(len(df[df['target']==-1])/len(df['target']))